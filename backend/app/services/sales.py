...
#         if sales_records:
#             db.bulk_insert_mappings(SalesData, [record.__dict__ for record in sales_records])
...
def get_sales_analytics(db: Session, restaurant_id: int, start_date: Optional[datetime] = None, end_date: Optional[datetime] = None):
    ...
    daily_sales = df.groupby('date_only').agg({
        'total_amount': 'sum',
        'date': 'count'  # This counts transactions
    ...
    # Top selling items
    top_items_df = df.groupby("item_name")["quantity"].sum().sort_values(ascending=False).head(10)
    top_items = [{"item_name": item, "total_quantity": qty} for item, qty in top_items_df.items()]
    ...
    hourly_data_raw = df.groupby("hour")["total_amount"].sum().to_dict()
    sales_by_hour = {str(hour): float(amount) for hour, amount in hourly_data_raw.items()}
    ...
    result = {
        ...
        "anomalies": [],  # Will be populated by LLM service
        "insights": []    # Will be populated by LLM service
    }
